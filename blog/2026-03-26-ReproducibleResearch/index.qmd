...
subtitle: "A Workflow with renv, resultcheck, Quarto, and GitHub"
author:
- name: "Dianyi Yang" 
...
links:
  - icon: newspaper
    name: Posit Open Source Blog
    url: https://opensource.posit.co/blog/2026-04-13_reproducible-research-renv-quarto-github/
---
...
# Result stability checks (`resultcheck`)

Once environments and file paths are reproducible, the next risk is *silent result drift*: code still runs, but key estimates or derived objects change without anyone noticing. To address this, the template now includes [`resultcheck`](https://github.com/kv9898/resultcheck/) as a core component alongside GitHub, `renv`, and Quarto.

The philosophy is simple (see the [package article](https://kv9898.github.io/resultcheck/articles/resultcheck.html)): treat important results as testable objects, not just things you glance at manually. With `resultcheck`, you save a snapshot for a key output once, then automatically compare later runs against that baseline. If a value changes unexpectedly, the check fails early and points you to what moved.

The most useful pattern is to start from day 1 and add snapshot checks whenever a script is "finished" for its stage. For example, once data cleaning is complete and you move on to analysis, add snapshot tests for cleaned data objects. When the analysis script stabilizes, snapshot model objects or regression tables. This creates a layered safety net as the project grows.

These checks work both locally and in GitHub-based testing workflows. In practice, that means pull requests can surface unintended changes in cleaned datasets, model estimates, or output tables before they reach the manuscript or slides. Since `resultcheck` is now published and relatively stable (with dedicated [documentation](https://kv9898.github.io/resultcheck/)), it is practical to make result verification part of routine project maintenance rather than a late-stage replication scramble.
